...
// 将 markdown slide 转化为 tag 格式：type 映射 + block 加 tag/style
var TYPE_MAP = {
...
function normalizeToTag(ast) {
  ...
  // ── 第二遍：标题去重 — 布局 slide 里第一级 heading 已被用作页面标题，从 blocks 中移除 ──
  ...
  if (title && isLayout) {
    for (var i = 0; i < blocks.length; i++) {
      ...
      if (headingText === title && (blocks[i].tag === 'h1' || blocks[i].tag === 'h2')) {
        blocks[i]._skip = true;  // 标记跳过，renderStack 会忽略
        break;  // 只移除第一个匹配的
      }
    }
  }

  // ── 第三遍：非布局 slide 自动堆叠坐标（PPT 导出需要）──
...
// TOC: 所有 heading 转成带编号的有序列表（第一项同时作页面标题）
