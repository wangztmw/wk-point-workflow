...
function render(ast, config) {
  ...
  const cardsHTML = items.map((item, i) => `
    <div style="flex:1;background:var(--color-bg-alt);padding:28px 20px;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;">
      ...
      <div style="font-size:36px;font-weight:800;color:#1a1a1a;line-height:1.1;font-family:'SF Mono','JetBrains Mono',monospace;">${escapeHTML(item.value)}</div>
      <div style="font-size:13px;color:#888;margin-top:8px;">${escapeHTML(item.label)}</div>
      ${item.trend ? `<div style="font-size:12px;color:${item.trend.startsWith('+')||item.trend.startsWith('↑')?'#16A34A':'#DC2626'};margin-top:4px;font-weight:600;">${escapeHTML(item.trend)}</div>` : ''}
    </div>
  `).join('');

  return `<div class="slide slide-kpi" style="background:var(--color-bg);padding:36px 44px;">
  <div class="section-title" style="font-size:24px;font-weight:700;color:#1a1a1a;margin-bottom:8px;">${escapeHTML(title)}</div>
