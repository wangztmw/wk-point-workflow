...
function render(ast, config) {
  ...
  const cardsHTML = cols.map((col, i) => `
    <div style="flex:1;background:var(--color-bg-alt);padding:24px 20px;display:flex;flex-direction:column;">
      ...
      <h4 style="font-size:17px;font-weight:700;color:#333;margin-bottom:10px;">${escapeHTML(col.title)}</h4>
      ${col.items.length > 0 ? `<ul style="list-style:none;">${col.items.map(item => `<li style="font-size:13px;color:#555;line-height:1.8;padding-left:0;">${escapeHTML(item)}</li>`).join('')}</ul>` : ''}
    </div>
