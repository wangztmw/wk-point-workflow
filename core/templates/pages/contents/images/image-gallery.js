...
function render(ast, config) {
  ...
  if (images.length === 0) {
    return `<div class="slide slide-content" style="background:var(--color-bg);padding:50px 70px;">
      <div class="section-title">${esc(title || '图片集')}</div><div class="divider"></div>
  ...
  const gridHTML = images.map((img, i) => {
    ...
    if (hasImage) {
      return `<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;">
        <div style="width:${cellW}px;height:${imgH}px;display:flex;align-items:center;justify-content:center;overflow:hidden;">
          <img src="${esc(img.src)}" style="max-width:100%;max-height:100%;object-fit:contain;" alt="${esc(label)}">
        </div>
        ${label ? `<div style="font-size:11px;color:#666;margin-top:4px;text-align:center;max-width:${cellW}px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${esc(label)}</div>` : ''}
      </div>`;
    }
    ...
    return `<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;">
      <div style="width:${cellW}px;height:${imgH}px;display:flex;align-items:center;justify-content:center;border:2px dashed #d0d0d0;background:#fafafa;">
        <div style="font-size:13px;font-weight:600;color:#999;text-align:center;max-width:${cellW - 20}px;line-height:1.3;">${esc(label)}</div>
      </div>
      ${label ? `<div style="font-size:11px;color:#999;margin-top:4px;text-align:center;max-width:${cellW}px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${esc(label)}</div>` : ''}
    </div>`;
  }).join('');

  return `<div class="slide slide-image-gallery" style="background:var(--color-bg);padding:${title ? '36px' : '24px'} 44px;">
  ${title ? `<div class="section-title" style="font-size:22px;font-weight:700;color:#1a1a1a;margin-bottom:12px;">${esc(title)}</div><div class="divider"></div>` : ''}
