...
demo-waterfall 项目 PPT 导出后瀑布图柱子之间的虚线连接混乱。HTML 端（ECharts）渲染正确，PPT 端（`tag-export.js` `renderWaterfallBars`）连接线错乱。连续 4 轮修改均未解决。

### 排查时间线

| 轮次 | 我的修改 | 思路 | 结果 |
|:--:|------|------|------|
| 1 | 照抄旧模板 `connectY`：增量柱连柱顶、减量柱连柱底 | 以为连接线是"柱和柱之间的连线"，给每个柱子算 `connectY` | 更乱了 |
| 2 | 增量 `connectY = barY + barH`，减量 `connectY = barY` | 微调 connectY 位置 | 还是错 |
| 3 | 增量连柱底（旧累计值），减量 `barY` 从旧累计值开始 | 调整减量柱的 barY 计算 | 部分连线对了、部分错了 |
| 4 | 放弃 `connectY` 追踪，改用 `linePoints` 数组 | **回头看 ECharts 版本怎么做的** | ✅ 一次修好 |

### 根因分析

**我陷入的思维死胡同**：

把连接线理解成"从前一根柱子的 `connectY` 到后一根柱子的 `connectY`"。增量柱和减量柱的 `connectY` 逻辑不同——增量柱连柱底（旧累计值位置），减量柱连柱底（新累计值位置）。每对柱子之间的 `prevConnectY` 和 `curConnectY` 高度可能不一致，线就乱跳。

这不是"算错了"，而是**模型本身错了**。

**正确的模型**：

连接线不是"柱和柱之间的连线"，而是**累计值水平线**。HTML/ECharts 版本用 `linePoints` 数组——`[500, 620, 700, 655, 685, 685]`——在 height=500 处画一条线从 bar0 到 bar1，在 height=620 处画一条从 bar1 到 bar2……每条线的高度由数据决定，跟柱子是增量还是减量完全无关。

```javascript
// 正确做法：照抄 HTML/ECharts 的 linePoints 逻辑
var linePoints = [rawData[0].value];
// ... 每步 push(cumulative) ...
// 连接线：在 linePoints[j] 高度从 bar[j] 到 bar[j+1]
...
用户反复说"增量柱的底部连接上一个柱子"、"减量柱的底部连接下一个柱子"——这些描述是正确的观察，但我在自己的错误框架里把"观察"当成了"算法指令"，拼命调整 `connectY` 的值。

最终用户说了一句关键的话："你在 HTML 里面是正确的"——这句话让我去读了 ECharts 版的代码，发现 `linePoints` 数组，5 分钟修好。

### AI 使用的教训

**1. 参考实现比算法描述高效**
用户描述"增量连底部、减量连底部"——我理解成算法指令，在自己的错误框架里修修补补。如果用户直接说"跟 HTML 版的 linePoints 对齐"，我一个 diff 就找到正确答案了。
...
用户跳过了第 2 步，直接给了第 3 步的算法描述。我就把观察当指令，没去读正确代码。如果有参考实现，指路比解释快十倍。

**3. 两个渲染端的同一功能，必须用同一数据模型**
HTML 端用 `linePoints` 画连接线，PPT 端也应该用 `linePoints` 画连接线。我试图发明一套 `connectY` 追踪逻辑来"对齐视觉效果"，结果越追越偏。数据模型统一 = 逻辑统一 = bug 无处藏身。

**4. 改了 3 轮还没对，立刻换思路**
第 1-3 轮都在微调 `connectY` 的位置。如果第 2 轮还没对，就应该怀疑整个模型错了，而不是继续微调参数。最终第 4 轮放弃 `connectY`、改用 `linePoints`，一行代码解决问题。
