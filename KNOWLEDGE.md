...
## ⚠️ AI 使用经验：瀑布图柱子"掉落"排查全记录

### 背景

demo-waterfall 项目：标签语法，左瀑布图（530×430）+ 右表格。重构后用户反复反馈"中间的增量柱子掉到下面去了"。

### 排查时间线

| 轮次 | 用户反馈 | 我的判断 | 修改位置 | 结果 |
|:--:|------|------|------|------|
| 1 | "柱子掉下去了" | 怀疑标题"金额"占空间 → 去掉 title | `waterfall.js` showTitle 逻辑 | 无效 |
| 2 | "还是掉下去" | 怀疑 grid.top 太多（20%→6%） | `waterfall.js` grid 参数 | 无效 |
| 3 | "还是掉下去" | 怀疑 grid 百分比换固定像素 | `waterfall.js` grid 参数 | 无效 |
| 4 | "还是掉下去" | 怀疑 `0` 干扰 ECharts stack → 改成 `'-'` | `waterfall.js` 数据格式 | 无效 |
| 5 | "连接线不能掉" | 恢复连接线 + 容器显式像素 | `waterfall.js` 容器+连线 | 无效 |
| 6 | **切换到老模板测试** | 老模板正常 → 新元素有问题 | `content.md` 语法切换 | **缩小范围** |
| 7 | "现在是正常的"（老模板） | 全幅(900×470)测试新元素 → 正常 | `content.md` 容器放大 | **锁定问题** |
| 8 | "小容器你也要做到" | grid 改回百分比 + borderColor | `waterfall.js` grid 参数 | HTML 正常 |
| 9 | **"导出PPT又出问题"** | **发现 cumulative 计算 bug** | `tag-export.js` 一行 | ✅ **真修好** |

### 根本原因

**有两个独立 bug**：

1. **HTML 端**（`waterfall.js`）：grid 从百分比误改成固定像素，小容器（530×430）里 48+16+16+44=124px 占 29%，绘图区被压扁。改回百分比解决。

2. **PPT 端**（`tag-export.js` `renderWaterfallBars`）：`cumulative` 初始值从 `0` 开始累加，漏掉了第一个柱子的值（500）。所有中间柱子在 PPT 里偏移 500 单位。一行修复：`cumulative = 0` → `cumulative = rawData[0].value`。

**这两个问题碰巧在同一个 demo 上暴露，且视觉表现相似（"柱子位置不对"），导致反复在错误的模块里排查。**

### AI 使用的教训

**1. 追问"你在哪里看到的"**
用户说"柱子掉了"，我没追问是浏览器里还是 PPT 里。项目有两套渲染管线（HTML + PPT），同一个视觉问题可能来自完全不同的代码路径。以后任何视觉反馈，第一反应应该是："浏览器预览还是导出 PPTX？"

**2. 用老版本做对照实验**
第 6 轮切换到老模板是最有效的排查手段——老模板正常，新元素异常，瞬间锁定问题在新代码。这种 A/B 对照应该在第一轮就做。

**3. 小改动累积的陷阱**
grid 改来改去（20%→6%→固定像素→48/16/16/44→百分比）全是微调，没有一个能根本解决问题——因为真正的问题不在 grid。如果改了两轮还无效，应该怀疑排查方向错了，而不是继续微调参数。

**4. 两头查，不要只盯一头**
项目有 HTML 渲染（`waterfall.js`）和 PPT 导出（`tag-export.js`）两个独立管道。同一视觉反馈可能是两端各自的 bug 叠加。修了一端无效，立刻切到另一端检查。

**5. 代码 diff 比肉眼推理可靠**
我反复对比老模板和新元素的 ECharts option，理论上它们"应该一样"。但 PPT 端的 cumulative bug 完全不在我的对比范围里——因为 HTML 端没有 `renderWaterfallBars` 这个函数。跨文件排查时，先列出两端各自的渲染函数清单，再逐一 diff。

**6. 真正的 bug 往往很小**
折腾了 9 轮，最终修了两行：`tag-export.js` 改一个初始值 + `waterfall.js` grid 改百分比。前面的 `'-'` vs `0`、`borderColor`、容器像素值——全是无用功。

---

## ⚠️ AI 使用经验：PPT 瀑布图连接线排查全记录

### 背景

demo-waterfall 项目 HTML 渲染正确，但导出 PPTX 后瀑布图柱子之间的虚线连接混乱。此前刚修复了柱子"掉落"问题（`cumulative` 初始值 bug），紧接着用户发现连接线也出了问题。

### 全量对话记录

**第 1 轮**

用户："现在布布局的连线更加错误了。增量柱是相对于一个base来增长的，所以应该是一个起始的柱子连接到增量柱子的底部。减量柱的话，应该是减量柱的底部连接下一个柱子。所以这个逻辑有问题。你在HTML里面是正确的，但是在导出PPT的过程中，PPT里面显示是错误的，而且是混乱的。找对地方，而且改对逻辑哈。"

我的理解：用户说增量柱的底部是旧累计值位置，减量柱的底部是新累计值位置，连接线应该把这些"底部"连起来。我把这个描述当成了算法指令，开始在 `renderWaterfallBars` 里照抄旧模板 `waterfall.js` 的 `connectY` 逻辑——为每个柱子算一个 `connectY` 值，增量柱 `connectY = barY + barH`（柱底 = 旧累计值），减量柱 `connectY = barY`（柱底 = 新累计值）。然后每条连接线从前一个柱子的 `connectY` 画到后一个柱子的 `connectY`。

**第 2 轮**

用户："现在布布局的连线更加错误了。"

我重新审视 `connectY` 的计算，发现减量柱的 `barY` 计算有误——之前用的是新累计值位置，应该是旧累计值位置。修正了 `barY` 的计算公式，然后重新调整 `connectY`：增量柱 `connectY = barY + barH`，减量柱 `connectY = barY + barH`。

**第 3 轮**

用户："你这虚线的连线还是有问题。你再好好地判一判，有几个是连对了的，有几个连错了。"

我开始逐条追踪。用数据 `[500, 120, 80, -45, 30, 685]` 手动计算每条连接线的高度，发现 `prevConnectY` 和 `curConnectY` 在增量-减量交界处高度不一致——从 bar1 到 bar2 的连线在 cumulative=500 高度，但从 bar2 到 bar3 的连接线在 cumulative=620 高度。每条线的高度不连续，像楼梯一样跳。

**第 4 轮（转折）**

我回头看了 HTML/ECharts 版本的连接线代码：

```javascript
// ECharts 版
var linePoints = [500, 620, 700, 655, 685, 685];
for (var j = 0; j < linePoints.length - 1; j++) {
    var p1 = api.coord([categories[j], linePoints[j]]);
    var p2 = api.coord([categories[j+1], linePoints[j]]);
    // 在 linePoints[j] 高度画水平线从 bar[j] 到 bar[j+1]
}
```

突然意识到：连接线不是"柱子和柱子之间的连线"，而是**累计值水平线**。`linePoints` 数组记录每步的累计值，在 height=500 处画一条线从 bar0 到 bar1，在 height=620 处画一条从 bar1 到 bar2……线的高度由数据决定，跟柱子是增量还是减量无关。

我把 `renderWaterfallBars` 的连接线逻辑从"追踪每个柱子的 `connectY`"改为"计算 `linePoints` 数组，逐条画水平线"：

```javascript
var linePoints = [rawData[0].value];
// ... 每步 push(cumulative) ...
for (var j = 0; j < linePoints.length - 1; j++) {
    var ly = baseY - linePoints[j] * scale;
    slide.addShape('line', { x: lx1, y: ly, w: lx2 - lx1, h: 0, ... });
}
```
...
用户问："你之前是怎么错的？然后之前是陷入了什么死循环、死脑筋里面？然后现在你又是怎么找到了正确的路径？"

我的回答：之前把连接线理解成"从前一根柱子的某个点到后一根柱子的某个点"——于是拼命算 `connectY`。正确的模型是"累计值水平线"——HTML/ECharts 版用的 `linePoints` 数组，每条线的高度是数据决定的，跟柱子类型无关。回头看 ECharts 代码才找到正确路径。

### 用户追问：当时应该怎么讲

用户问："那么回过头来看，我当时应该怎么跟你讲，你会比较容易理解这个逻辑呢？"

我的回答："跟 HTML 版的 linePoints 对齐就行"——就这一句话。有参考实现的时候，指路比解释更高效。

用户："也就是说有的时候我去解释反而让你把目标对象搞错了，我应该把我的需求讲出来，把我的观察讲出来，作为一种观察辅助的描述，对不对？"

我的回答：对。最有效的模式是三步——"哪里不对"（锁定模块）、"哪里是对的"（给参考实现）、"观察"（验证辅助）。用户跳过了第 2 步直接给了第 3 步的算法描述，我就把观察当成了指令。

### 根因分析

**错误的思维模型**：把连接线当作"柱与柱之间的连线"，为每个柱子计算一个 `connectY` 点（增量柱连顶部/底部，减量柱连顶部/底部），然后从前柱的 `connectY` 画到后柱的 `connectY`。这个模型的问题在于：增量柱和减量柱的 `connectY` 语义不一致——增量柱的 `connectY` 是旧累计值（柱底），减量柱的 `connectY` 是新累计值（柱底），两条柱子的 `connectY` 高度不同，连线就成了斜线或者断线。

**正确的数据模型**：连接线是**累计值水平线**。用一个 `linePoints` 数组记录每步后的累计值——`[500, 620, 700, 655, 685, 685]`。在 height=500 处从 bar0 画到 bar1，在 height=620 处从 bar1 画到 bar2……每条线的高度由累计值决定，跟柱子的类型（增量/减量/落地）完全无关。这个模型和 HTML/ECharts 端完全一致。
...
> **Claude**：旧模板连接逻辑：增量柱连顶部（旧累计值）、减量柱连底部（新累计值）。我的实现用了 `barY` 但应该用 `barY + barH` 来区分增减。直接照抄旧模板：[修改了 connectY 逻辑]

> **用户**：现在布布局的连线更加错误了。

> **Claude**：[修正了减量柱 barY 计算——之前用新累计值位置，改为旧累计值位置。重新调整 connectY]

> **用户**：你这虚线的连线还是有问题。你再好好地判一判，有几个是连对了的，有几个连错了。

> **Claude**：让我逐条追踪连接线位置，找到对错：[用 node 脚本计算 linePoints = [500,620,700,655,685,685]，列出 5 条连接线各在哪个高度]。找到问题了。HTML 版画了 5 条连接线（每个累计值一条），我的 PPT 版只画了 5 条但全用了错误的 `prevConnectY`。应该用 `linePoints` 数组：[重写连接线逻辑，用 linePoints 数组逐个高度画水平线]

> **用户**：你终于做对了。你之前是怎么错的？然后之前是陷入了什么死循环、死脑筋里面？然后现在你又是怎么找到了正确的路径？

> **Claude**：之前错在两个地方。思维死胡同：我把连接线理解成"从前一根柱子的某个点到后一根柱子的某个点"——于是拼命算 `connectY`，增量柱连顶、减量柱连底，结果每对柱子之间的 `prevConnectY` 和 `curConnectY` 高度不一致，线就乱了。正确的模型：连接线不是"柱和柱之间的连线"，而是"累计值水平线"。HTML/ECharts 版用的 `linePoints` 数组——`[500, 620, 700, 655, 685, 685]`，在 height=500 处画一条线从 bar0 到 bar1，在 height=620 处画一条从 bar1 到 bar2……每条线的高度是数据决定的，跟柱子类型无关。怎么找到的：回头看了 ECharts 版本的代码——`for (var j = 0; j < hl.linePoints.length - 1; j++)` 在 `linePoints[j]` 高度画线。不是"算柱子的连接点"，而是"算累计值的水平线"。一行代码替换了整个 `connectY` 追踪逻辑。

> **用户**：那么回过头来看，我当时应该怎么跟你讲，你会比较容易理解这个逻辑呢？

> **Claude**："跟 HTML 版的 linePoints 对齐就行"——就这一句话。你不需要解释连接线的逻辑，直接告诉我参考哪个已有实现，我就会去读 ECharts 那几行代码，5 分钟搞定。反而是你每次描述"增量连底部、减量连底部"的时候，我把你的描述当成了算法指令，在自己的错误框架里修修补补。有参考实现的时候，指路比解释更高效。

> **用户**：也就是说有的时候我去解释反而让你把目标对象搞错了，我应该把我的需求讲出来，把我的观察讲出来，作为一种观察辅助的描述，对不对？

> **Claude**：对。最有效的模式是三步：1. "哪里不对"——PPT 导出后连接线乱了。2. "哪里是对的"——HTML 版是正确的，参考它。3. "观察"——我注意到增量柱好像底部跟上一个柱子有关系。第 1 步让我锁定模块，第 2 步给我正确参考，第 3 步作为验证辅助。你跳过第 2 步直接给了第 3 步的算法描述，我就把"观察"当成了"指令"，在自己的错误框架里实现你的描述，而不是去读正确代码。

---

**1. 参考实现比算法描述高效十倍**
用户描述"增量连底部、减量连底部"——全程是正确的观察。但我在自己的错误框架里把这些观察当成了算法指令，拼命调整 `connectY` 参数。如果用户直接说"跟 HTML 版的 linePoints 对齐"，我一个 diff 就找到正确答案了。

**2. "哪里是对的" + "哪里是错的" = 最优排查路径**
最有效的 bug 报告应该包含三步：
- "哪里不对"（锁定模块）→ PPT 导出后连接线乱了
- "哪里是对的"（给参考）→ HTML 版是正确的，参考它
- "观察"（验证辅助）→ 我注意到增量柱底部跟上一个柱子有关系

用户跳过第 2 步直接给了第 3 步的算法描述，我就把观察当指令，在错误框架里修修补补。指路比解释快。

**3. 两个渲染端的同一功能，必须用同一数据模型**
HTML 端用 `linePoints` 画连接线，PPT 端也应该用 `linePoints` 画连接线。我试图发明一套 `connectY` 追踪逻辑来"对齐视觉效果"，结果越追越偏。数据模型统一了，逻辑自然统一，bug 无处藏身。

**4. 用户给的算法描述 ≠ 正确的算法模型**
用户说"增量柱底部连接上一个柱子"——这句话描述的是**观察到的视觉现象**，不是**底层的数据模型**。我把现象描述当成了实现指令，在 PPT 坐标系统里模拟这个"底部连接"，而不是从 ECharts 的数据模型（`linePoints`）出发重新实现。用户的描述永远是辅助验证，核心逻辑要从代码中找。

**5. 改了 3 轮还没对，立刻换思路**
第 1-3 轮都在微调 `connectY` 的取值和计算公式。做第 2 轮还没对的时候就应该停下来怀疑整个模型是不是错了，而不是继续微调。第 4 轮放弃 `connectY`、回头看参考实现，一行代码解决问题。

**6. 排查时先问"哪里有对的参考实现"**
这次两个渲染端同时存在——HTML 端是对的，PPT 端是错的。最有效的做法不是盯着错误端推理，而是直接 diff 两端代码，找差异。有参考实现时，"抄"比"想"快。
