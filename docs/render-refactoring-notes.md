...
从 masonry 瀑布流开始暴露——HTML 用 `overflow:hidden` 自动裁掉溢出文字，PPT 没有裁剪机制，文字超出 rect 就换行→高度增加→和下一个元素重叠。

### 错误尝试 1：硬编码安全系数

给 `blockHeight` 加 `PPT_FACTOR=1.25`，所有高度 ×1.25。结果：有些页文字太小，有些页仍然重叠。**死因：固定系数不能适配不同内容的实际高度差异。**

### 错误尝试 2：逐块调字号

masonry 给每个 block 单独调字号，超标者缩小。结果：h3 字号不一致（18px/14px/12px 混排），视觉突兀。**死因：应该按 tag 分组统一缩放，保持同类元素一致。**

### 错误尝试 3：用估算高度判断遮挡

`blockHeight` 用 `colW*96/(fs*1.0)` 算每行字符数→算行数→算高度。但拉丁字符实际宽 0.55×字号，中文字符 1.0×字号。估算把混合文本的行数算少了。判断"没重叠"但实际已重叠。**死因：估值不准，判断偏差。**

### 突破 1：blockHeightActual ——逐字算实际宽度

```js
function textWidth(text, fs) {
  for each char: isWide(c) ? fs * 1.0 : fs * 0.55;
}
function blockHeightActual(block, colW):
  text → pixW / (colW*96) = 真实行数 → 行数 × fs/96 × LINE_H + padding
  非text → blockHeight(block, colW)
```

### 突破 2：全局等比缩放

不再逐块调整。全部元素同比例缩小，直到适配。同类 tag 保持一致。

### 突破 3：迭代收敛

排列 → 检验实际高度 → 有溢出? → 全局 ×0.92 → 重排 → 再检验。最多 3 轮。

### 突破 4：内容居中

`center.js`：排列完成后计算实际包围盒（所有 block 的 min/max x/y），对比页面内容区，差值/2 = dx/dy 偏移。

### 突破 5：PPT 文本截断

`elements.js truncatePPT()`：在生成 `_ppt` 后，计算 rect 能容纳的最大字符数，超出则截断加 `…`。从源头杜绝 PPT 文本溢出。

### 缩放参数总结

| 参数 | 值 | 理由 |
|------|-----|------|
| LINE_H | 2.0 | PPT 比 HTML 高约 10-15%，2.0 兼顾两者 |
| 放大目标 | pageH × 0.85 | 留 15% 空白更美观 |
| 放大上限 | 1.3× | 避免表格/长文本因字号过大换行 |
| 缩小下限 | 0.5× | 保证可读性 |
| 收敛轮数 | 3 | 3 轮足够收敛到稳定态 |
...
| 改前 | 改后 |
|------|------|
| `blockHeight` 固定 1.4" | 有图: `min(colW×0.75, 3.0")`（按 4:3），无图: 0.8" |
| split 权重 wgt=3 | wgt=2（不过度偏袒右侧） |
| 列宽无上限 | 最多 6.5"，最少 3.0" |
