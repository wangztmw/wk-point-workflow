 ...
 *   - 项目立项
 */
const nodeEl = require('../../elements/composite/timeline-node');
...
function render(ast, config) {
  ...
  const nodesHTML = nodes.map((node, i) => {
    ...
    return `<div style="position:absolute;left:${cardX}px;top:${cardY}px;">${nodeEl.render(node.date, node.items, node.imageSrc, nodeW, nodeH)}</div>`;
  }).join('');
