...
function render(ast, config) {
  ...
  if (!hasImage) {
    return `<div class="slide slide-content" style="background:var(--color-bg);padding:50px 70px;display:flex;flex-direction:column;align-items:center;justify-content:center;">
      <div style="display:flex;align-items:center;justify-content:center;width:400px;height:250px;border:2px dashed #d0d0d0;background:#fafafa;">
        <div style="font-size:22px;font-weight:600;color:#999;text-align:center;">${esc(label)}</div>
      </div>
      ${title ? `<div style="font-size:18px;color:#666;margin-top:16px;">${esc(title)}</div>` : ''}
    </div>`;
  }

  return `<div class="slide slide-image-full" style="background-image:url(${esc(imgData.src)});background-size:cover;background-position:center;position:relative;display:flex;align-items:center;justify-content:center;">
  ...
  <div style="position:relative;z-index:1;text-align:center;color:#fff;padding:40px 60px;">
    ${title ? `<div style="font-size:36px;font-weight:800;margin-bottom:12px;letter-spacing:2px;text-shadow:0 2px 8px rgba(0,0,0,0.4);">${esc(title)}</div>` : ''}
    ${sub ? `<div style="font-size:18px;font-weight:300;opacity:0.85;margin-bottom:8px;">${esc(sub)}</div>` : ''}
    ${desc ? `<div style="font-size:14px;opacity:0.7;margin-top:12px;max-width:500px;margin-left:auto;margin-right:auto;">${esc(desc)}</div>` : ''}
